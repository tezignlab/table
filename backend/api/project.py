...
from api.user import table_user, get_user
...
table_project = mongo['table']['project']
table_project_collection = mongo['table']['project-collection']
table_project_collect = mongo['table']['project-collect']
table_project_like = mongo['table']['project-like']
table_project_view = mongo['table']['project-view']
...
async def _extra_project_field(request: Request, project_list: List):
    ...
    if user_id:
        ...
        user_likes = {x['project_id'] for x in list(table_project_like.find(_filter, project_ID_PROJECTION))}
    for item in project_list:
        ...
        if user_id:
            ...
            collection_ids = [x['collection_id'] for x in list(table_project_collect.find({'user_id': user_id, 'project_id': item['id']}, {'_id': 0, 'collection_id': 1}))]
            collection_ids = [ObjectId(item) for item in collection_ids]
            collections = table_project_collection.find({'_id': {'$in': collection_ids}})
...
def _collections_with_cover(user_id: str, cover_size: int):
    ...
    """
    collections = list(table_project_collection.aggregate([{
    ...
    project_map = {str(x['_id']): x['cover'] for x in table_project.find({'_id': {'$in': project_ids}}, {'_id': 1, 'cover': 1})}
...
async def projects(request: Request, skip: int = 0, limit: int = 20):
    ...
    result = list(table_project.find(_filter, project_PROJECTION).skip(skip).limit(limit).sort([('publish_time', -1)]))
    total = table_project.count_documents(_filter)
...
async def project_detail(request: Request, project_id: str):
    ...
    doc = table_project.find_one({'_id': ObjectId(project_id)}, projection)
    doc['id'] = str(doc['_id'])
    user_id = await optional_user_id(request)
    if user_id:
        doc['is_like'] = table_project_like.find_one({'user_id': user_id, 'project_id': project_id}) is not None
        
        # find projects collections
        collection_ids = [x['collection_id'] for x in list(table_project_collect.find({'user_id': user_id, 'project_id': doc['id']}, {'_id': 0, 'collection_id': 1}))]
        collection_ids = [ObjectId(item) for item in collection_ids]
        collections = table_project_collection.find({'_id': {'$in': collection_ids}})
        collections = [{**item, 'id': str(item['_id'])} for item in collections]
...
async def project_search(request: Request, task: BackgroundTasks, keyword: str, skip: int = 0, limit: int = 20):
    ...
    docs = list(table_project.find({'_id': {'$in': [ObjectId(x) for x in project_ids]}}, project_PROJECTION))
...
async def project_recommend(request: Request, skip: int = 0, limit: int = 20):
    cursor = table_project.aggregate([{
        '$match': {
            'origin': {"$ne": 'topys'}
        },
    }, {
...
async def likes(request: Request, user_id: str, skip: int = 0, limit: int = 20):
    ...
    result = list(table_project_like.find(_filter, project_ID_PROJECTION).skip(skip).limit(limit).sort([('time', -1)]))
    total = table_project_like.count_documents(_filter)
    if not result:
        return Response(data=PageData())
    project_ids = [ObjectId(x['project_id']) for x in result]
    docs = list(table_project.find({'_id': {'$in': project_ids}}, project_PROJECTION))
...
async def do_like(project_id: str, user_id: str = Depends(current_user_id)):
    ...
    result = table_project_like.update_one(_filter, doc, upsert=True)
    if result.upserted_id:
        table_project.update_one({'_id': ObjectId(project_id)}, {'$inc': {'count_like': 1}})
    return Response()
...
async def cancel_like(project_id: str, user_id: str = Depends(current_user_id)):
    result = table_project_like.delete_one({'user_id': user_id, 'project_id': project_id})
...
async def create_collection(body: CollectionBody, user_id: str = Depends(current_user_id)):
    ...
    result = table_project_collection.insert_one(body)
...
async def update_collection(collection_id: str):
    collection = table_project_collection.find_one({'_id': ObjectId(collection_id)})
    user = table_user.find_one({'_id': ObjectId(collection['user_id'])})
...
async def update_collection(collection_id: str, body: CollectionBody, user_id: str = Depends(current_user_id)):
    ...
    table_project_collection.update_one({'_id': ObjectId(collection_id), 'user_id': user_id}, {'$set': body})
    return Response()
...
async def delete_collection(collection_id: str, user_id: str = Depends(current_user_id)):
    table_project_collection.delete_one({'_id': ObjectId(collection_id), 'user_id': user_id})
    table_project_collect.delete_many({'user_id': user_id, 'collection_id': collection_id})
    return Response()
...
async def collect_list(request: Request, collection_id: str, skip: int = 0, limit: int = 20):
    ...
    result = list(table_project_collect.find(_filter, project_ID_PROJECTION).skip(skip).limit(limit).sort([('time', -1)]))
    total = table_project_collect.count_documents(_filter)
    if not result:
        return Response(data=PageData(skip=skip, limit=limit, total=0, has_more=False, data=[]))
    project_ids = [ObjectId(x['project_id']) for x in result]
    docs = list(table_project.find({'_id': {'$in': project_ids}}, project_PROJECTION))
...
async def collection_list(project_id: str, user_id: str = Depends(current_user_id)):
    collection_ids = [x['collection_id'] for x in list(table_project_collect.find({'user_id': user_id, 'project_id': project_id}, {'_id': 0, 'collection_id': 1}))]
...
async def do_collect(project_id: str, collection_id: str, user_id: str = Depends(current_user_id)):
    ...
    table_project_collect.update_one(_filter, doc, upsert=True)
    return Response()
...
async def cancel_collect(project_id: str, collection_id: str, user_id: str = Depends(current_user_id)):
    table_project_collect.delete_one({'user_id': user_id, 'project_id': project_id, 'collection_id': collection_id})
    return Response()
...
async def do_view(request: Request, project_id: str):
    ...
    if user_id:
        _filter = {'user_id': user_id, 'project_id': project_id}
        doc = {'$set': {'user_id': user_id, 'project_id': project_id, 'time': TimeUtils.now_timestamp()}}
        table_project_view.update_one(_filter, doc, upsert=True)
    table_project.update_one({'_id': ObjectId(project_id)}, {'$inc': {'count_read': 1}})
...
async def views(request: Request, skip: int = 0, limit: int = 20, user_id: str = Depends(current_user_id)):
    ...
    result = list(table_project_view.find(_filter, project_ID_PROJECTION).skip(skip).limit(limit).sort([('time', -1)]))
    total = table_project_view.count_documents(_filter)
    if not result:
        return Response(data=PageData(skip=skip, limit=limit, total=0, has_more=False, data=[]))
    project_ids = [ObjectId(x['project_id']) for x in result]
    docs = list(table_project.find({'_id': {'$in': project_ids}}, project_PROJECTION))
...
async def delete_view(project_id: str, user_id: str = Depends(current_user_id)):
    table_project_view.delete_one({'user_id': user_id, 'project_id': project_id})
    return Response()
