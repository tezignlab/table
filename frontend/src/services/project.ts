...
export const collectProject = async (
  ...
  (await axios[method](`/api/v1/project/${projectId}/collect/${collectionId}`))
    .data
...
export const getCollectionDetailList = async (
  ...
  (await axios.get(`/api/v1/project/collect/${id}?skip=${skip}&limit=${limit}`))
    .data
...
export const getRecommendProjects = async (
  ...
  (await axios.get(`/api/v1/project/recommend?skip=${skip}&limit=${limit}`))
    .data
