...
export default function SignUp() {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            email: Yup.string()
                .required(`${t('auth.validation.require')}${t('user.email')}`)
                .matches(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i, `${t('auth.validation.correct')}${t('user.email')}`),
            password: Yup.string()
                .required(`${t('auth.validation.require')}${t('user.password')}`)
                .min(
                    PASSWORD_MIN_LENGTH,
            ...
            confirmPassword: Yup.string()
                ...
                .when('password', {
                    ...
                    then: Yup.string().oneOf([Yup.ref('password')], `${t('auth.validation.correct')}${t('user.confirmPassword')}`),
                }),
        }),
    })
            ...
            {!authStatus.success && (
                <FormikProvider value={formik}>
                    ...
                    <Form className="flex flex-col space-y-6 text-left">
                        ...
                        />
                        <Input label={t('user.confirmPassword')} id="confirmPassword" name="confirmPassword"
                               type="password"/>
